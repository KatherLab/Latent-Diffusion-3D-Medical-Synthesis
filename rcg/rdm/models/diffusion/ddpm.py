...
class RDM(DDPM):
    ...
    def instantiate_pretrained_enc(self, config):
        ...
        # self.pretrained_encoder = UNetModel(dims=3, image_size=128, in_channels=2,
        #           model_channels=96, out_channels=2, 
        #           num_res_blocks=1, attention_resolutions=[32,16,8],
        #           channel_mult=[1, 1, 2, 2])

        self.pretrained_encoder = UNetModel(dims=3, image_size=96, in_channels=2,
                        model_channels=96, out_channels=2, 
                        num_res_blocks=1, attention_resolutions=[32,16,8],
        ...
        load_state_dict(self.pretrained_encoder, "/share/project/zhaohuxing/nvidia_generation/maisi/logs_four_datasets/representation_ep50_96/autoencoder.pt")
    ...
    def process_rep(self, rep):
        b, c, d, h, w = rep.shape
        rep = rep.reshape((b, c, d*h*w))
        rep = rep.mean(dim=-1)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        ## at this time, x is (batch, 4, d, h, w)
        ...
        with torch.no_grad():
            ...
            rep_source = self.pretrained_encoder.image_encode(x[:, :2])
            rep_target = self.pretrained_encoder.image_encode(x[:, 2:])

            rep_source = self.process_rep(rep_source)
            rep_target = self.process_rep(rep_target)
            

        rep_target = rep_target.unsqueeze(-1).unsqueeze(-1)
        # x = x.cuda()
        z = rep_target

        rep_source = rep_source.unsqueeze(-1).unsqueeze(-1)
        c = rep_source
