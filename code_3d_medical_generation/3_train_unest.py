...
def collate_fn(batch):
    """
    Collate function to combine a list of dicts into a dict of batched tensors/lists.
    Assumes each item in the batch is a dict with the same keys.
    """
    if not isinstance(batch[0], dict):
        raise TypeError("Each item in the batch should be a dict.")
    ...
    for key in batch[0]:
        try:
            ...
            # For non-tensor values like strings or lists
...
args = OmegaConf.load("configs/config_unest.yaml")
...
def train(local_rank, world_size):
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        with tqdm(total=len(dataloader_train), disable=(local_rank != 0)) as t:
            ...
            # print(f"start iterate the dataloader.......")
