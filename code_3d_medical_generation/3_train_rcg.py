...
def train(local_rank, world_size):
    ...
    model = UNetModel(dims=3, image_size=96, in_channels=2,
                    model_channels=96, out_channels=2, 
                    num_res_blocks=1, attention_resolutions=[32,16,8],
                    channel_mult=[1, 2, 2, 2]).to(device)
        ...
        with tqdm(total=len(dataloader_train), disable=(local_rank != 0)) as t:
            ...
            for batch in dataloader_train:
                ...
                with autocast(enabled=True):
                    # Encode source modality as conditioning for RDM
                    with torch.no_grad():
                        rdm.pretrained_encoder.eval()
                        source_rep = rdm.pretrained_encoder.image_encode(images)
                    ...
                    reconstruction = model(images, sampled_rep)

                    loss_re = intensity_loss(reconstruction, labels)
                    
                    loss_p = loss_perceptual(reconstruction[:, 0:1].float(), labels[:, 0:1].float()) + loss_perceptual(reconstruction[:, 1:2].float(), labels[:, 1:2].float()) 
