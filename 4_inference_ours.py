...
config_file = "./configs/config_unet3d_rcg.yaml"
...
model = load_state_dict(model, "./logs/unet3d_rcg_ep1000_test/autoencoder.pt")
model.eval()
...
save_dir = "./predictions/generated_data_unet3d_rdm_ep500/"
    ...
    for batch in tqdm(val_data, total=len(val_data)):
        ...
        with torch.no_grad():
            with autocast(enabled=True):
                ...
                print(f"images is {images.shape}")
                reconstruction = val_inferer(images, model, rdm_rep=sampled_rep)
...
inference(val_files_brats21, "predictions/brats21_rcg")
